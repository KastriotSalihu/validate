...
data_set::data_set(file_names files)
	: instance(files.instance_path), solution(files.solution_path), error_check(this->N)
{
...
void data_set::read_solution_file(string solution)
{
	...
	if (wf.is_open())
	{
		...
		while (getline(wf, line))
		{
			if (process_solution_lines(line, line_number));
...
bool data_set::process_solution_lines(string line, int line_number)
{
	vector<int> values = split_string_to_ints(line);

	bool solution_integrity = error_check.test_solution_format(line, line_number, values);
...
void data_set::verify()
{
	...
	for (auto map_iterator = fleets.begin(); map_iterator != fleets.end(); map_iterator++)
	{
		...
		for (ride *current_ride : map_iterator->second)
		{
			...
			status = travel(current_ride, vehicle_position_row, vehicle_position_col, elapsed_time, score);

			if (status != "") error_check.record_travel_errors(status, map_iterator->first, current_ride_index);
